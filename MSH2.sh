...
VCF=$2 # the result from gatk filtering

# get the read count line from result of sambamba
VARIANT=`grep '2[[:space:]]47641560' $DEP`
ARR=($(echo "$VARIANT"|tr '\t' '\n'))

# If there is reads at the position: 2:47641560
if [ ! -z "${ARR[0]}" ]; then
    DP=`expr ${ARR[2]} - ${ARR[7]}`
    COUNTA=`echo ${ARR[3]}|cut -f2 -d':'`
    COUNTC=`echo ${ARR[4]}|cut -f2 -d':'`
    COUNTG=`echo ${ARR[5]}|cut -f2 -d':'`
    COUNTT=`echo ${ARR[6]}|cut -f2 -d':'`
